...
<head>
    <meta charset="UTF-8">
    <title>MorasPrestamosIndividual</title>
    <style type="text/css">
        * {
...
<body>
    ...
    @foreach ($prestamos as $key => $value)
        <div class="encabezado">
            ...
            <h4 class="texto">MORAS PRÉSTAMOS INDIVIDUAL</h4>
        ...
        <table border="1" class="cliente">
            <tbody>
                <tr>
                    ...
                    <td class="bold gray">C.I.: </td>
                    <td>{{ $value->cliente->full_ci }}</td>
                </tr>
                <tr>
        ...
        <h4 class="centreado font-md">LISTA DE PAGOS CON MORA</h4>
        @php
            $plan_pagos_mora = App\Models\PlanPago::where('fecha_pago', '<', date('Y-m-d'))
                ->where('prestamo_id', $value->id)
                ->where('cancelado', 'NO')
                ->get();
        @endphp
        <table border="1">
            ...
            <tbody>
                @php
                    $total = 0;
                @endphp
                @foreach ($plan_pagos_mora as $ppm)
                    @php
                    ...
                    <tr>
                        ...
                        <td class="centreado">{{ $dias_mora }}</td>
                        <td class="centreado">{{ number_format((float) $monto_mora, 2, '.', ',') }}</td>
                    </tr>
                @endforeach
                <tr>
                    <td class="bold gray font-md derecha" colspan="2">TOTAL</td>
                    <td class="bold gray font-md centreado">{{ number_format($total, 2, '.', ',') }}</td>
                </tr>
            </tbody>
        </table>
